...
- [x] **CSV 試算表 Excel 核心功能升級**：
  - **標題即時篩選 (Header Filter)**：工具列新增「🔍 標題篩選」切換按鈕，每一直欄下方提供專屬輸入框，即時過濾匹配資料列。
  - **經典 Excel 儲存格拖曳下拉填滿 (Fill Handle)**：點選儲存格時右下角顯示拖曳把手，支援純文字向下複製、數值與編號（如 `1`、`001`、`A-1`）自動遞增序列；拖曳時具備虛線高亮預覽。
  - **儲存格右鍵快捷選單 (Cell Context Menu)**：支援一鍵「向下填滿相同內容 (至最後一列)」、「向下自動遞增填滿 (+1, +2...)」與「清空此儲存格」。
